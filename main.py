...
crypto_combo2 = None  # переменная для выпадающего списка для выбора криптовалюты
crypto_choose_win = None  # переменная для окна отображения выпадающего списка выбора криптовалюты
...
crypto_names = {}  # словарь наименований криптовалют
...
# Функция для считывания из файла json библиотеки с названиями криптовалют и их id для api
def get_crypto_dict():
    global crypto_names
...
# Функция проверки ввода данных в поля конвертера: можно только точку и цифры
...
# Функция получения курса криптовалют и вывода на экран курса и информации о дате и времени последнего обновления
def get_rate():
    ...
    # Если в поле пусто, по умолчанию считается, что идёт расчет за 1 единицу криптовалюты.
    if amount_text == '':
        amount_text = '1.00'
        crypto_amount.insert(0,amount_text)
        mb.showwarning('Внимание!',f'Курс рассчитан за 1 единицу {crypto}')

    # Если в поле максимум 1 точка, приводится текст к вещественному типу и выполняется расчет, иначе выводится сообщение об ошибке.
    if amount_text.replace('.','',1).isalnum():
        amount_ = float(amount_text)
        try:
            response = requests.get(f'https://api.coingecko.com/api/v3/simple/price?ids={crypto_id}&vs_currencies='
                                    f'{t_currency_id}&include_last_updated_at=true')
            response.raise_for_status()
            counter += 1
            counter_lbl.config(text=f'Использовано запросов: {counter}')

            data = response.json()
            exchange_rate = data[crypto_id][t_currency_id]  # определяется курс криптовалюты
            crypto_cur_lbl.config(text=f'Курс криптовалюты: 1 {crypto_names[crypto][1]} = {exchange_rate:.5f} {t_currency_id.upper()}')
            last_upd_t = data[crypto_id]['last_updated_at']  # время последнего обновления курса криптовалюты
...
# Функция для обновления данных в конвертере
def update_rate():
    if exchange_rate:
        answer = sd.askinteger('Уточните,',
                               f'что нужно пересчитать:\n\n 1 - курс криптовалюты в выбранной валюте\n '
                               f'2 - количество криптовалюты, которое можно приобрести за указанную сумму')
        if answer == 1:
            recalc_cur()
        elif answer == 2:
            recalc_crypto()
        else:
            mb.showerror('Ошибка',f'Введено неверное значение "{answer}"')
    else:
        get_rate()


# Функция пересчета количества валюты, которое будет стоить введенное количество криптовалюты.
def recalc_cur():
...
# Функция пересчета количества криптовалюты, которое можно приобрести за введенное количество валюты.
def recalc_crypto():
...
# Функция сохранения полученного курса криптовалюты в текстовый файл.
def save_rate():
...
# Функция отображения сохраненных курсов криптовалют.
def show_history():
...
# Функция закрытия главного окна (выход из программы)
def exit_w():
    window.destroy()


# Функция добавления новой криптовалюты
def add_crypto():
    global crypto_names
    global counter
    found_cryptos = {}
    ...
    if new_crypto:
        ...
        try:
            response = requests.get(coins_list_url)  # обращение по api к списку криптовалют
            response.raise_for_status()
            counter += 1
            counter_lbl.config(text=f'Использовано запросов: {counter}')
            data = response.json()

            # Если введенное название соответствует id криптовалюты или её полному/сокращенному названию,
            # и если id найденной криптовалюты не входит в список id криптовалют, задействованных в приложении,
            # то данные о найденной криптовалюте включаются во временный словарь found_cryptos
            for coin in data:
            ...
            # Если найдена только 1 криптовалюта, выводится сообщение с предложением её добавления.
            # В случае согласия пользователя, данные добавляются в словарь, обновляется файл json.
            ...
            # Если криптовалюты, соответствующие условиям, не найдены, возможно, они уже ей в словаре программы, или
            # пользователь указал неверное название
            ...
            # В случае нахождения нескольких криптовалют, выводится соответствующее сообщение.
            # Пользователю необходимо уточнить более полное название
            else:
                mb.showwarning('Внимание',f'Найдено несколько ({len(found_cryptos)}) криптовалют\n\n '
                                          f'"Сокращение (Полное_название)":\n{'\n'.join([name for name in found_cryptos.keys()])},'
                                          f'\n\n уточните название')
        except Exception as e:
...
# Функция обновления файла json с данными (названия) об используемых в программе криптовалютах.
def update_crypto_dict_f():
     with open(crypto_dict_file,'w') as f:
        json.dump(crypto_names, f, indent=4)


# Функция выбора криптовалюты для вывода информации о ней.
def choose_crypto():
    global crypto_combo2
    global crypto_choose_win
    crypto_choose_win = Toplevel(window)
    crypto_choose_win.protocol('WM_DELETE_WINDOW', hide_win)  # при нажатии на "крестик", окно будет скрываться
...
# Функция сокрытия дочернего окна
def hide_win():
    crypto_choose_win.withdraw()


# Функция отображения информации о криптовалюте.
def show_info():
    global counter
    ...
    # Блок виджетов для отображения основной информации о выбранной криптовалюте
    Label(show_win, text='Основная информация о криптовалюте', font='Arial 14 bold',bg='white').grid(
        row=0,column=0,columnspan=5,sticky='ew',pady=10, padx=10)
    Label(show_win,text='Название криптовалюты:',font='Arial 10', width=28).grid(row=1,column=0,padx=5)
    Label(show_win, text=f'{crypto_names[crypto_name][2]}',font='Arial 10 bold', bg='white',width=15,borderwidth=2,relief='ridge').grid(
        row=1,column=1,ipadx=5,ipady=2,padx=2)
    Label(show_win, text='Сокращённое название:',font='Arial 10').grid(row=2,column=0)
    Label(show_win, text=f'{crypto_names[crypto_name][1]}',font='Arial 10 bold', bg='white',width=15,borderwidth=2,relief='ridge').grid(
        row=2,column=1,ipadx=5,ipady=2,padx=2)
    ...
    # Блок виджетов для отображения текущего курса и логотипа
    ...
    # Блок виджетов для отображения исторических значений
    ...
    # Блок виджетов для отображения изменения цены криптовалюты в % по периодам
    ...
    try:
        ...
        # Заполнение меток полученными данными о криптовалюте
        ...
        # Изменение цвета %-значений: если цена выросла, зеленый цвет, если снизилась или отсутствует информация, красный.
        ...
        # Если от api получен логотип, осуществляется его загрузка.
        if ac.logo != '-':
            logo_l.config(image=ac.logo)
            logo_l.image = ac.logo

    except Exception as e:
        mb.showerror('Ошибка', f'Возникла ошибка: {e}')


# Функция сохранения полного списка криптовалют в файл.
def save_crypto_list():
    ...
    try:
        ...
        counter_lbl.config(text=f'Использовано запросов: {counter}')
        data = response.json()
        # Символ и название каждой криптовалюты из списка криптовалют добавляются в соответствующие списки.
        for coin in data:
            coin_symbols.append(coin['symbol'])
            coin_names.append(coin['name'])
        # Создаётся элемент DataFrame
        df = pd.DataFrame({
            'Сокращение': coin_symbols,
            'Наименование': coin_names
        })
        # Элемент DataFrame сохраняется в файл excel.
...
filemenu.add_command(label="Закрыть", command=exit_w)
