...
def exist_data_check(data): # Проверка, есть ли данные и округление при наличии
    if data:
...
def exist_data_percent_check(data): # Проверка, есть ли данные в процентах и округление при наличии
    if data:
        if isinstance(data,float):
            return round(data,2)
        else:
...
def about(data): # получение параметров криптовалюты
    ...
    logo = load_image(logo_url) if logo_url else '-'
    print(logo)


def load_image(logo_url): # загрузка логотипа
    if logo_url:
        try:
